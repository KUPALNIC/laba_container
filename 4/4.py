...
with open('/home/kupalnic/CLionProjects/laba_container/cmake-build-debug/4.txt', 'r') as file:
    for line in file:
...
plt.title('Среднее время удаления элемента из начала для std::list, std::forward_list, and Subforwardlist')
plt.xlabel('Container Size')
plt.ylabel('Время удаления элемента, мкс')
plt.legend()
plt.grid(True)

# Show plot
plt.savefig('pop_front_time_comparison.png')
plt.show()